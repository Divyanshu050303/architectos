...
from .rules import completeness, configuration, consistency, policy, requirements


def default_registry() -> Registry:
    """A new registry each time: a registry can still be registered into, so none is shared."""
    return Registry(
        [*consistency.RULES, *completeness.RULES, *configuration.RULES, *policy.RULES, *requirements.RULES]
    )
