...
|---|---|---|---|
| Routes | One architecture per project: `/projects/{id}/architecture/…` | Many: `/projects/{id}/architectures` (list, create) and `/projects/{id}/architectures/{architectureId}/…` | Add an architecture picker; use the plural routes |
| Architecture body | `{id, projectId, version, nodes, edges, assumptions, createdAt, createdBy}` | `{id, projectId, name, description, status, currentVersion, …, revision: {version, source, summary, contentHash, …}, ir, layout}` with the IR document in `ir` | Read `ir.nodes`, `ir.connections`, `ir.assumptions`; `createdBy` becomes `revision.source`; show `name` and `status` |
| Metadata, lifecycle | None | `PATCH` name/description (no revision); archive, restore, delete (soft, archived only) | Rename and archive from the architecture list |
| Restore | None | `POST …/versions/{n}/restore {baseVersion}` creates a new revision with revision n's content | Offer "restore this version" in the history view |
...
| Versions | `GET …/versions` → array | `{versions, nextCursor}` (newest first), each with `current` and `restoredFromVersion` | Follow `nextCursor` |
...
| Save response | New `Architecture` | `201` new revision + `created: true` + `changes` (the diff); `200` + `created: false` when nothing changed | Use `changes` to highlight what changed |
| Whole save | None | `PUT …/content {baseVersion, ir}` | Available for imports and full saves |
